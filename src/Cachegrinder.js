...
export class Cachegrinder {
	static read(path) {
		return new Promise((resolve, reject) => {
			new FileReader(path).read().then(fileReader => {
				...
				while (!fileReader.end()) {
					if (fileReader.line().match(new RegExp('^fl='))) {
						...
						let callableModel = collection.hasCallable(fn[1]) ? collection.getCallable(fn[1]) :
							new Callable(fn[1], fn[2].trim() === '{main}' ? fl[2].trim() : fn[2].trim(), fnInfo[1], fnInfo[2]);
